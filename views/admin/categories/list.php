					...
					<?php foreach ($categories as $cat): ?>
						<tr class="border-b hover:bg-gray-50 transition">
							...
							<td class="px-6 py-4 flex gap-3">
								<a href="<?= BASE_URL ?>/admin/categories/<?= $cat['id'] ?>/edit" class="text-blue-600 hover:underline">Sửa</a>
								<a href="<?= BASE_URL ?>/admin/categories/<?= $cat['id'] ?>/delete" class="text-red-600 hover:underline" onclick="return confirm('Bạn có chắc muốn xóa?')">Xóa</a>
							</td>
						</tr>
					<?php endforeach; ?>
				</tbody>
