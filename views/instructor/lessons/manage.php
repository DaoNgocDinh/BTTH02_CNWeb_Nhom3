					...
					<?php foreach ($lessons as $lesson): ?>
						<tr class="border-b hover:bg-gray-50 transition">
							...
							<td class="px-6 py-4 flex gap-3 flex-wrap">
								...
								<form action="<?= BASE_URL ?>/instructor/lesson/upload/<?= $lesson->id ?>" method="POST" enctype="multipart/form-data" class="flex gap-2">
									<input type="hidden" name="lesson_id" value="<?= $lesson->id ?>">
									<input type="file" name="files[]" multiple required class="text-sm" />
									<button type="submit" class="text-green-600 hover:underline">Upload</button>
								</form>
							</td>
						</tr>
					<?php endforeach; ?>
